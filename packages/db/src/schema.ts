...
export const publicationStatus = pgEnum("es_publication_status", [
  "DRAFT",
  "PENDING_REVIEW",
  "APPROVED",
  "SCHEDULED",
...
export const verificationStatus = pgEnum("es_verification_status", [
  "UNVERIFIED",
  "SOURCE_CONFIRMED",
  "COMPANY_VERIFIED",
  "LINK_CONFIRMED",
...
export const originType = pgEnum("es_origin_type", [
  "MANUAL",
  "SPREADSHEET",
  "API",
  "COMPANY_SUBMISSION",
...
export const queueStatus = pgEnum("es_queue_status", [
  "PENDING",
  "PROCESSING",
  "COMPLETED",
  "FAILED",
...
export const cities = pgTable(
  "es_cities",
  {
    ...
    metropolitanRegion: boolean("metropolitan_region").notNull().default(false),
...
export const neighborhoods = pgTable(
  ...
  (t) => [uniqueIndex("es_neighborhoods_city_slug_uq").on(t.cityId, t.slug)]
);
...
export const roles = pgTable("es_roles", {
  id: uuid("id").primaryKey().defaultRandom(),
  key: text("key").notNull().unique(),
  name: text("name").notNull(),
  ...timestamps
});
export const permissions = pgTable("es_permissions", {
  id: uuid("id").primaryKey().defaultRandom(),
  key: text("key").notNull().unique(),
  description: text("description"),
  ...timestamps
...
export const userRoles = pgTable(
  ...
  (t) => [uniqueIndex("es_user_roles_uq").on(t.userId, t.roleId)]
);
export const rolePermissions = pgTable(
  ...
  (t) => [uniqueIndex("es_role_permissions_uq").on(t.roleId, t.permissionId)]
);
...
export const candidateSessions = pgTable("es_candidate_sessions", {
  id: uuid("id").primaryKey().defaultRandom(),
  userId: uuid("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
...
export const userJobPreferences = pgTable("es_user_job_preferences", {
  userId: uuid("user_id")
    .primaryKey()
    .references(() => users.id, { onDelete: "cascade" }),
  cities: jsonb("cities").notNull().default([]),
  categories: jsonb("categories").notNull().default([]),
  workplaceTypes: jsonb("workplace_types").notNull().default([]),
...
export const loginAttempts = pgTable(
  ...
  (t) => [index("es_login_attempts_rate_idx").on(t.emailHash, t.ipHash, t.createdAt)]
);
export const jobs = pgTable(
  "es_jobs",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    publicCode: varchar("public_code", { length: 9 }).notNull().unique(),
    ...
    workplaceType: text("workplace_type").notNull().default("presencial"),
    employmentType: text("employment_type").notNull(),
    numberOfOpenings: integer("number_of_openings").notNull().default(1),
    salaryMin: numeric("salary_min"),
    salaryMax: numeric("salary_max"),
    salaryCurrency: varchar("salary_currency", { length: 3 }).notNull().default("BRL"),
    ...
    featured: boolean("featured").notNull().default(false),
    confidentialCompany: boolean("confidential_company").notNull().default(false),
    unidentifiedCompany: boolean("unidentified_company").notNull().default(false),
    sponsored: boolean("sponsored").notNull().default(false),
  ...
  (t) => [
    index("es_jobs_publication_idx").on(t.publicationStatus, t.publishedAt),
    index("es_jobs_location_idx").on(t.stateId, t.cityId),
    uniqueIndex("es_jobs_external_source_uq").on(t.externalId, t.sourceName)
  ]
);
export const savedJobs = pgTable(
  ...
  (t) => [uniqueIndex("es_saved_jobs_uq").on(t.userId, t.jobId)]
);
export const jobRevisions = pgTable(
  ...
  (t) => [uniqueIndex("es_job_revisions_uq").on(t.jobId, t.version)]
);
export const jobSources = pgTable("es_job_sources", {
  id: uuid("id").primaryKey().defaultRandom(),
  jobId: uuid("job_id")
    .notNull()
    .references(() => jobs.id),
...
export const authors = pgTable("es_authors", {
  id: uuid("id").primaryKey().defaultRandom(),
  name: text("name").notNull(),
  slug: text("slug").notNull().unique(),
  bio: text("bio"),
...
export const articles = pgTable(
  "es_articles",
  {
    ...
    coverImageVariants: jsonb("cover_image_variants").notNull().default({}),
...
export const articleRevisions = pgTable(
  ...
  (t) => [uniqueIndex("es_article_revisions_uq").on(t.articleId, t.version)]
);
export const mediaAssets = pgTable("es_media_assets", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const brandAssetHistory = pgTable(
  "es_brand_asset_history",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const redirects = pgTable("es_redirects", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const importBatches = pgTable(
  ...
  (t) => [index("es_import_batches_hash_idx").on(t.fileHash, t.createdAt)]
);
export const importMappingTemplates = pgTable("es_import_mapping_templates", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const importRows = pgTable(
  ...
  (t) => [uniqueIndex("es_import_rows_uq").on(t.batchId, t.rowNumber)]
);
...
export const backgroundJobs = pgTable("es_background_jobs", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const publicationSchedules = pgTable("es_publication_schedules", {
  id: uuid("id").primaryKey().defaultRandom(),
  name: text("name").notNull(),
  status: text("status").notNull().default("ACTIVE"),
  configuration: jsonb("configuration").notNull(),
  jobIds: jsonb("job_ids").notNull(),
...
export const shortLinks = pgTable("es_short_links", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const socialPosts = pgTable("es_social_posts", {
  id: uuid("id").primaryKey().defaultRandom(),
  entityType: text("entity_type").notNull(),
...
export const socialPublications = pgTable("es_social_publications", {
  id: uuid("id").primaryKey().defaultRandom(),
  postId: uuid("post_id")
    .notNull()
...
export const subscriptions = pgTable("es_subscriptions", {
  id: uuid("id").primaryKey().defaultRandom(),
  email: text("email").notNull().unique(),
...
export const alerts = pgTable("es_alerts", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const notificationDeliveries = pgTable("es_notification_deliveries", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const consentLogs = pgTable("es_consent_logs", {
  id: uuid("id").primaryKey().defaultRandom(),
...
export const advertisers = pgTable("es_advertisers", {
  id: uuid("id").primaryKey().defaultRandom(),
  name: text("name").notNull(),
...
export const campaigns = pgTable("es_campaigns", {
  ...
  internalNotes: text("internal_notes"),
  disclosure: text("disclosure").notNull().default("Publicidade"),
  ...timestamps
});
export const adSlots = pgTable("es_ad_slots", {
  ...
  priority: integer("priority").notNull().default(0),
  allowAdsense: boolean("allow_adsense").notNull().default(true),
  allowDirect: boolean("allow_direct").notNull().default(true),
  adsenseSlotId: text("adsense_slot_id"),
  exclusionRules: jsonb("exclusion_rules").notNull().default({}),
  reservedHeight: integer("reserved_height").notNull().default(280),
  sortOrder: integer("sort_order").notNull().default(0),
  ...timestamps
});
...
export const adEvents = pgTable(
  ...
  (t) => [
    index("es_ad_events_reporting_idx").on(t.slotKey, t.event, t.createdAt),
    index("es_ad_events_campaign_idx").on(t.campaignId, t.event, t.createdAt)
  ]
);
export const indexingEvents = pgTable(
  ...
  (t) => [index("es_indexing_event_queue_idx").on(t.status, t.createdAt)]
);
export const settings = pgTable("es_system_settings", {
...
export const orderStatus = pgEnum("es_order_status", [
  "DRAFT",
  "PENDING_PAYMENT",
  "PAID",
  "CANCELLED",
...
export const companyUserRole = pgEnum("es_company_user_role", [
  "OWNER",
  "MANAGER",
  "RECRUITER",
  "BILLING",
...
export const refundStatus = pgEnum("es_refund_status", [
  "REQUESTED",
  "APPROVED",
  "PROCESSING",
  "COMPLETED",
...
export const commercialOrders = pgTable(
  ...
  (t) => [
    index("es_commercial_orders_status_idx").on(t.status, t.createdAt),
    index("es_commercial_orders_email_idx").on(t.email)
  ]
);

export const commercialPayments = pgTable(
  "es_commercial_payments",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (t) => [
    index("es_commercial_payments_order_idx").on(t.orderId, t.status),
    index("es_commercial_payments_status_idx").on(t.status, t.createdAt)
  ]
);

export const commercialPaymentEvents = pgTable(
  "es_commercial_payment_events",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const commercialRefunds = pgTable(
  "es_commercial_refunds",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const companyCredits = pgTable(
  ...
  (t) => [
    index("es_company_credits_email_idx").on(t.email),
    index("es_company_credits_company_idx").on(t.companyId)
  ]
);
...
export const companyJobDrafts = pgTable(
  "es_company_job_drafts",
  {
    id: uuid("id").primaryKey().defaultRandom(),
...
export const companyTickets = pgTable(
  "es_company_tickets",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    accountId: uuid("account_id").references(() => companyAccounts.id),
...
export const seoAuditIssues = pgTable(
  ...
  (t) => [
    index("es_seo_audit_issues_url_idx").on(t.url, t.checkKey),
    index("es_seo_audit_issues_open_idx").on(t.resolved, t.ignored, t.severity)
  ]
);

export const jobRelations = relations(jobs, ({ one, many }) => ({
  company: one(companies, { fields: [jobs.companyId], references: [companies.id] }),
  city: one(cities, { fields: [jobs.cityId], references: [cities.id] }),
  revisions: many(jobRevisions),
