...
const schema = z.object({
  ...
  next: z.string().trim().max(400).optional().or(z.literal(""))
});
...
export const POST: APIRoute = async ({ request, url, redirect }) => {
  ...
  try {
    await connection.db
      .insert(users)
      .values({ email, name: parsed.data.name || email.split("@")[0]!, active: true })
      .onConflictDoNothing();
    const [user] = await connection.db.select().from(users).where(eq(users.email, email)).limit(1);
    if (!user) throw new Error("Usuário não encontrado.");
    const [administrativeRole] = await connection.db
    ...
    await connection.db.insert(magicLinkTokens).values({
      userId: user.id,
      tokenHash: candidateTokenHash(token),
      expiresAt: new Date(Date.now() + 15 * 60 * 1000)
    });
    const [delivery] = await connection.db
      .insert(notificationDeliveries)
      .values({ channel: "EMAIL", template: "MAGIC_LINK" })
      .returning();
    const access = new URL("/acesso", url.origin);
    access.searchParams.set("token", token);
    if (nextPath) access.searchParams.set("next", nextPath);
    ...
    try {
      await queue.add(
        "email",
        {
          ...
          to: email,
          subject: "Seu acesso — Empregos São Luís",
          html: `<p>Use o link abaixo em até 15 minutos:</p><p><a href="${access.toString()}">Entrar na minha conta</a></p><p>Se não solicitou, ignore.</p>`
        },
