...
export const GET: APIRoute = async () => {
  ...
  checks.storage = storage.exists && storage.read && storage.write && storage.delete ? "ok" : "error";
  if (checks.storage !== "ok") ok = false;
  return Response.json({ ok, service: "web", status: ok ? "ready" : "not_ready", checks }, { status: ok ? 200 : 503, headers: { "cache-control": "no-store" } });
};
