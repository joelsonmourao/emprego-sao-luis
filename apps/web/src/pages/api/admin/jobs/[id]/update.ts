...
export const POST: APIRoute = async ({ params, request, locals, clientAddress }) => {
  ...
  if (!version.success || !parsed.success) {
    return adminJsonError("Revise os campos da vaga.", 422, {
  ...
  try {
    ...
    await connection.db.transaction(async (tx) => {
        ...
        .set({
          ...
          publishedAt:
            parsed.data.publicationStatus === "PUBLISHED"
              ...
              : before.publishedAt,
          duplicateHash: createHash("sha256")
            .update(`${parsed.data.normalizedTitle}|${parsed.data.companyId}|${parsed.data.cityId}`)
            .digest("hex"),
          version: before.version + 1,
          updatedAt: new Date()
        })
        .where(and(eq(jobs.id, before.id), eq(jobs.version, version.data)))
        .returning();
      ...
      await tx.insert(auditLogs).values({
        ...
        entityId: before.id,
        before,
        after: { record: saved, ip: clientAddress, userAgent: request.headers.get("user-agent") },
        origin: "ADMIN"
      });
    ...
    logServerError("route:/api/admin/jobs/:id/update", error, {
      userId: auth.id,
      entity: "JOB",
      route: `/api/admin/jobs/${params.id}/update`
    });
    const message =
      error instanceof Error && error.message.includes("Conflito")
        ? error.message
