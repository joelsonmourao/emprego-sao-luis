...
import { monitorApplicationUrls } from "./application-monitor.js";
...
async function main() {
  ...
  await maintenanceQueue.upsertJobScheduler("monitor-application-links", { every: 60 * 60 * 1000 }, { name: "monitor-application-links", data: {} });
  const maintenanceWorker = new Worker("maintenance", async (job) => job.name === "expire-jobs" ? expireJobs() : job.name === "publish-scheduled" ? publishScheduledJobs() : job.name === "dispatch-alerts" ? dispatchJobAlerts() : job.name === "monitor-application-links" ? monitorApplicationUrls() : processIndexingEvents(), { connection: redisConnection, concurrency: 1 });
