/**
 * Reset manual do cache Geoapify para permitir reprocessamento com Google Places API (New).
 *
 * Uso:
 *   npm run enrich:locations:reset-geoapify-cache:dry-run -- --latest-jobs=2000
 *   npm run enrich:locations:reset-geoapify-cache -- --latest-jobs=2000
 */
...
const GEOAPIFY_PROVIDER_ID = "geoapify";
...
async function main() {
  ...
  console.info(`=== Reset cache Geoapify ${dryRun ? "— DRY-RUN" : "— REAL"} ===`);
  ...
  console.info(`Caches Geoapify no recorte: ${counts.total}`);
  ...
  console.info(`Caches Geoapify apagados: ${deleted.count}`);
}

main()
  .catch((error) => {
    console.error("Reset Geoapify interrompido por erro fatal:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
