...
import { buildCompanyListingSeo, buildListingCollectionPageJsonLd, buildListingFaq, getCityJobsPath, getCompanyJobsPath, getStateJobsPath } from "@/lib/seo/jobs-pages";
...
function buildPageHref(slug: string, order: "relevance" | "date" | undefined, pageNumber: number) {
  ...
  return query ? `${getCompanyJobsPath(slug)}?${query}` : getCompanyJobsPath(slug);
}
...
export async function generateMetadata({
  ...
  if (!company) {
    return buildSiteMetadata({
      ...
      pathname: getCompanyJobsPath(slug),
      noIndex: true
    });
  }
...
export default async function CompanyJobsPage({
  ...
  const relatedCompanies = allCompanies.filter((item) => item.slug !== company.slug && (item.citySlug === company.city.slug || item.stateSlug === company.state.slug)).slice(0, 8);
    ...
    <section className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      <JsonLd
        data={buildBreadcrumbJsonLd([
          ...
          { name: company.name, path: getCompanyJobsPath(company.slug) }
        ])}
      />
          ...
          <div className="brand-panel rounded-[1.8rem] border border-slate-200 p-6 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {relatedCompanies.map((item) => (
                <Link key={item.slug} href={getCompanyJobsPath(item.slug)} className="rounded-full border border-[color:rgba(26,43,76,0.1)] bg-white px-4 py-2 text-sm font-medium text-[var(--brand-text-secondary)] transition hover:text-[var(--brand-orange)]">
                  {item.name}
                </Link>
              ))}
            </div>
