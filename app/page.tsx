  ...
  const pageSections: Record<(typeof homeBlockKeys)[number], ReactNode> = {
    quickAccess: (
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="grid gap-6 lg:grid-cols-2">
          <Link href="/vagas" className="brand-chip rounded-[2rem] p-7 transition hover:-translate-y-1 hover:shadow-[0_28px_80px_-52px_rgba(26,43,76,0.4)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-black text-[var(--brand-navy)]">{siteContent.home.quickJobsTitle}</h2>
                <p className="mt-3 max-w-xl text-base leading-7 text-[var(--brand-text-secondary)]">{siteContent.home.quickJobsDescription}</p>
              </div>
              <BriefcaseBusiness className="h-10 w-10 text-[var(--brand-orange)]" />
            </div>
          </Link>

          <Link
            href="/blog"
            className="rounded-[2rem] border border-[color:rgba(255,109,0,0.18)] bg-[linear-gradient(145deg,#1a2b4c_0%,#21406e_56%,#2f6fed_100%)] p-7 text-white shadow-[0_30px_90px_-55px_rgba(26,43,76,0.68)] transition hover:-translate-y-1"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
              ...
              <Newspaper className="h-10 w-10 text-[var(--brand-orange)]" />
            </div>
          </Link>
        </div>
      </section>
          ...
          {siteContent.home.howItWorksSteps.map((step) => {
            ...
            return (
              <div key={step.title} className="brand-chip rounded-[1.75rem] p-6">
                <Icon className="h-7 w-7 text-[var(--brand-orange)]" />
                <h2 className="mt-4 text-2xl font-black text-[var(--brand-navy)]">{step.title}</h2>
                <p className="mt-2 text-sm leading-7 text-[var(--brand-text-secondary)]">{step.description}</p>
              </div>
            );
          })}
        </div>
    ...
    citiesAndBenefits: (
      <section className="mx-auto max-w-7xl space-y-8 px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-2">
          <div className="brand-panel rounded-[2rem] border border-slate-200 p-8 shadow-[0_25px_90px_-54px_rgba(26,43,76,0.24)]">
            <SectionHeading
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {cities.slice(0, 8).map((city) => (
                <Link
                  key={city.id}
                  href={`/vagas/estado/${city.state.slug}/${city.slug}`}
                  className="rounded-full border border-[color:rgba(26,43,76,0.1)] bg-white px-4 py-2 text-sm font-medium text-[var(--brand-text-secondary)] transition hover:border-[color:rgba(255,109,0,0.28)] hover:text-[var(--brand-orange)]"
                >
                  {city.name}, {city.state.code}
                </Link>
              ))}
          ...
          <div className="brand-dark-panel rounded-[2rem] p-8 text-white shadow-[0_30px_100px_-50px_rgba(26,43,76,0.8)]">
            <SectionHeading
            ...
            <div className="mt-6 grid gap-5">
              {siteContent.home.benefits.slice(0, 3).map((item) => {
                ...
                return (
                  <div key={item.title} className="rounded-[1.5rem] border border-white/18 bg-white/10 p-5 shadow-[0_18px_50px_-35px_rgba(15,23,42,0.4)] backdrop-blur">
                    <Icon className="h-7 w-7 text-[var(--brand-orange)]" />
          ...
          {siteContent.home.careerCtas.map((item) => {
            ...
            return (
              <Link key={item.title} href={item.href as Route} className="brand-chip rounded-[2rem] p-6 transition hover:-translate-y-1">
                <Icon className="h-8 w-8 text-[var(--brand-orange)]" />
                <h3 className="mt-5 text-2xl font-black text-[var(--brand-navy)]">{item.title}</h3>
                <p className="mt-2 text-sm leading-7 text-[var(--brand-text-secondary)]">{item.description}</p>
              </Link>
            );
          })}
        </div>
    ...
    companies: (
      <section className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="brand-chip rounded-[2rem] p-6">
            <h2 className="text-lg font-black text-[var(--brand-navy)]">{siteContent.home.companiesTitle}</h2>
            <p className="mt-2 text-sm leading-7 text-[var(--brand-text-secondary)]">{siteContent.home.companiesDescription}</p>
            <div className="mt-5 grid gap-3">
              {companies.slice(0, 6).map((company) => (
                <Link
                  key={company.slug}
                  href={`/empresas/${company.slug}`}
                  className="rounded-[1.25rem] border border-[color:rgba(26,43,76,0.1)] bg-white px-4 py-4 text-sm text-[var(--brand-text-secondary)] transition hover:border-[color:rgba(255,109,0,0.24)] hover:text-[var(--brand-orange)]"
                >
    ...
    finalCta: (
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="rounded-[2.5rem] bg-[linear-gradient(135deg,#1a2b4c_0%,#21406e_44%,#2f6fed_100%)] p-8 text-white">
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-orange-100">{siteContent.home.finalCtaEyebrow}</p>
